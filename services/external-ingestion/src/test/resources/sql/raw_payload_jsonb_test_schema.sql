-- Test-only schema for ExternalApiRawPayloadJsonbTest.
-- Uses JSONB for external_api_raw_payload JSON columns (live DB contract).
-- Incorporates V1-V5 schema changes into a single baseline.
...
-- JSONB for all JSON columns: live DB contract differs from deprecated V1 (TEXT).
...
-- Incorporates V3 (nullable coords/capacity), V4 (audit cols), V5 (canonical status)
CREATE TABLE shelter (
    ...
    shelter_status VARCHAR(20)    NOT NULL DEFAULT 'OPERATING'
                   CONSTRAINT chk_shelter_status CHECK (shelter_status IN ('OPERATING', 'STOPPED', 'PREPARING')),
    note           TEXT,
    created_at     TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    updated_at     TIMESTAMPTZ    NOT NULL DEFAULT NOW()
);
