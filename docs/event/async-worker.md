...
> 이벤트 envelope 구조 및 SQS payload 전문은 `docs/event/event-envelope.md`를 참조한다.
...
3. Worker 처리 흐름      ← 각 worker의 의사코드
4. Output 포맷           ← Redis SET 결과물
...
| `CacheRegeneration` | Redis miss 발생 키 재생성 | `CacheRegenerationRequested` | `api-public-read`가 Redis miss + suppress window 통과 후 발행. cacheKey prefix 기준으로 worker 분기 | `cache-worker` / `readmodel-worker` | read-path fallback 이후 캐시 복구 |
...
> **재난 Redis 키 정의 (유일한 정의)** 재난 관련 Redis 키는 아래 3종으로 고정한다.
...
| --- | --- | --- | --- | --- | --- | --- |
| `RecalculateShelterStatus` | 사용자 조회용 대피소 현재인원·잔여인원·혼잡도 계산 | `shelterId` | `shelter:status:{shelterId}` | 30초 | RDS COUNT() 후 SET | RDS가 source of truth. 별도 카운터 컬럼 유지 시 트랜잭션 실패로 정합성 붕괴 위험. 재난 상황 즉각 대응을 위해 사용자 TTL을 30초로 설정 |
| `RebuildDisasterReadModel` | 지역·재난 유형별 활성 목록·상세 전체 재구성 | `region`, `disasterType` | `disaster:active:{region}` / `disaster:alert:list:{region}:{disasterType}` / `disaster:detail:{alertId}` | 2분 / 5분 / 10분 | 수집 결과 정규화 후 SET. `hasExpiredAlerts: true` 시 `disaster:active:{region}` DEL 후 재생성 | 수집 주기(분 단위) 대비 TTL을 동등 이상으로 설정하여 수집 완료 전 stale 노출 최소화 |
| `RebuildEnvironmentCache` | 격자 좌표·측정소 기반 날씨·대기질 데이터 재구성 | 좌표 / 측정소 | `env:weather:{nx}:{ny}` / `env:air:{station_name}` | 120분 | 외부 API 결과 정규화 후 SET | 날씨·대기질은 수집 주기가 길고(1시간 단위) 변화가 느림. stale-while-revalidate 패턴 적용. 외부 API fallback 제거로 인해 캐시 유효성이 더 중요 |
...
### `cache-worker` / `readmodel-worker` — CacheRegenerationRequested

```
1. SQS 메시지 수신 (CacheRegenerationRequested)
2. idempotencyKey 검사
   └─ 이미 처리된 키면 → no-op 후 ACK
3. cacheKey prefix 기준 분기
   ├─ shelter:status:{shelterId}
   │    → cache-worker
   │    → RDS COUNT() 재계산 후 Redis SET shelter:status:{shelterId} (TTL 30초)
   │
   ├─ disaster:alert:list:{region}:{disasterType}
   │    → readmodel-worker
   │    → RDS에서 해당 region + disasterType 알림 목록 조회
   │    → Redis SET disaster:alert:list:{region}:{disasterType} (TTL 5분)
   │
   ├─ disaster:latest:{disasterType}:{region}
   │    → readmodel-worker
   │    → RDS에서 해당 유형 + 지역 최신 alertId 조회
   │    → Redis SET disaster:latest:{disasterType}:{region} (pointer, TTL 5분)
   │
   └─ disaster:detail:{alertId}
        → readmodel-worker
        → RDS에서 해당 alertId 상세 조회
        → Redis SET disaster:detail:{alertId} (TTL 10분)
4. ACK
5. 실패 시 → 재시도 (Exponential Backoff) → 한도 초과 시 DLQ 이동
```

---

## 4. Output — Redis SET 포맷

### EVENT-001 / EVENT-002 — RecalculateShelterStatus 결과

```json
// shelter:status:{shelterId}  (TTL 30초)
{
  "currentOccupancy": 69,
...
### EVENT-004 — ShelterUpdated 후 RecalculateShelterStatus 결과
...
### EVENT-005 — RebuildDisasterReadModel 결과
...
### EVENT-006 — RebuildEnvironmentCache 결과
...
### EVENT-007 — CacheRegenerationRequested 결과
...
// disaster:latest:{disasterType}:{region}  (TTL 5분)
{
  "alertId": 55
}

// disaster:detail:{alertId}  (TTL 10분)
{
  "alertId": 55,
  "disasterType": "FLOOD",
  "region": "서울특별시",
...
> **async+worker 영역 정의** async+worker는 `api-core` 또는 compute 영역의 `external-ingestion Normalizer`가 발행한 SQS 이벤트를 소비하는 **Lambda 워크로드**다. SQS 캐시 갱신 큐의 소유 및 소비 주체는 async+worker 영역이며, Redis 키 갱신 및 Read Model 재구성을 수행한다. 실패 시 재시도 후 DLQ로 이동한다.

### 5-1. 컴포넌트별 책임 분리

| 컴포넌트 | 실행 환경 | 주요 책임 | 입력 | 출력 |
| --- | --- | --- | --- | --- |
| `cache-worker` | Lambda | SQS 이벤트 소비, 대피소 상태 캐시 재생성, 환경 캐시 재구성, CacheRegenerationRequested shelter 키 처리 | SQS — 도메인 이벤트, 수집 완료 이벤트, read-path 보상 이벤트 | Redis 상태 키 SET |
| `readmodel-worker` | Lambda | SQS 이벤트 소비, 재난 조회 Read Model 전체 재구성, CacheRegenerationRequested disaster 키 처리 | SQS — `DisasterDataCollected`, `CacheRegenerationRequested` (disaster 키) | Redis `disaster:active:*`, `disaster:alert:list:*`, `disaster:detail:*`, `disaster:latest:*` SET / DEL |

### 5-2. 컴포넌트 경계 — 하지 않는 것

| 컴포넌트 | 하지 않는 것 | 이유 |
| --- | --- | --- |
| `external-ingestion Normalizer` | Redis 직접 접근 | 수집 완료 이벤트 발행으로 역할 종료. Redis 갱신은 async+worker Lambda 전담 |
...
| CacheRegenerationRequested | Redis timeout / RDS 조회 실패 | 3회 | 1s → 5s → 30s | ✅ |
...
| `CacheRegenerationRequested` | 허용 | `cacheKey + windowStart` 기준 — suppress window 단위 dedupe. 동일 키 중복 수신 시 overwrite |
...
`cache-worker`는 사용자 조회용 대피소 상태 캐시, 환경 캐시, read-path 보상 이벤트의 shelter 키 처리를 담당하고, `readmodel-worker`는 재난 조회 Read Model 및 read-path 보상 이벤트의 disaster 키 처리를 담당한다.
...
| 날짜 | 항목 | 내용 |
| --- | --- | --- |
| 2026-04-20 | `RebuildEnvironmentCache` TTL | 60분 → 120분. stale-while-revalidate 패턴 적용 |
| 2026-04-23 | `CacheRegenerationRequested` | EVENT-007 신규 추가. api-public-read read-path 보상 이벤트 명세 및 worker 분기 처리 추가 |
| 2026-04-23 | `EvacuationEntryUpdated` idempotencyKey | `entry:{entryId}:UPDATED` → `entry:{entryId}:UPDATED:{eventId}` |
| 2026-04-23 | `ShelterUpdated` idempotencyKey | `shelter:{shelterId}:UPDATED` → `shelter:{shelterId}:UPDATED:{eventId}` |