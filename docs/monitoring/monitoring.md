맞습니다. async-worker 담당 범위가 곧 `SQS/Lambda`면 문서에서 그 영역을 “참고용 infra”로만 밀면 안 됩니다. 아래 버전은 `Application 담당`과 `Async-worker(SQS/Lambda) 담당`이 각각 구현/운영해야 할 metric/log를 분리했고, SQS/Lambda를 async-worker의 1차 책임 영역으로 넣었습니다.
